    ...
    try {
      shot = await callFal(
        "fal-ai/bytedance/seedream/v4/edit",
        {
          ...
          prompt:
            "CRITICAL BACKGROUND: The background MUST be PURE WHITE #FFFFFF (RGB 255,255,255) — absolutely no cream, no beige, no ivory, no off-white, no warm tint, no cool tint, no gray, no gradient, no vignette, no paper texture, no shadow on the background itself. The four corners of the canvas must be exactly #FFFFFF. If in doubt, make the background BRIGHTER and WHITER, not warmer. Move the exact same product onto this pure white #FFFFFF seamless studio background. Keep the product identical to the input image: preserve every printed label, logo, brand name, illustration, color, material and proportions exactly as in the source — do NOT redraw, restyle or remove any packaging text or graphics that are physically printed on the product. CRITICAL FRAMING: scale the product UP so it fills 70–75% of the frame in BOTH width and height — the longest edge of the product must span about 75% of the canvas. Center the product both horizontally and vertically with equal small margins on all four sides (~12–15% of canvas). Do NOT leave large empty white space around the product, do NOT push the product to the bottom or top, do NOT render it small in the middle of an empty canvas. Add a soft realistic contact shadow directly under the product (shadow only under the product, never tinting the background). WATERMARK REMOVAL: remove any watermarks, store logos, website URLs, photo credits, shop names (e.g. 'bestgun.pl', 'militaria', 'gun-center', 'e-militaria') and any semi-transparent overlay text that are NOT physically printed on the product packaging itself. Keep only graphics and text that physically exist on the product/packaging. Sharp focus, professional e-commerce product photography, accurate colors. Avoid: cream background, beige background, ivory background, off-white background, warm background, gray background, any background that is not exact #FFFFFF, any tint or color cast on the background, vignette, paper texture on the background, missing labels, blurred text, regenerated artwork, tiny product, product smaller than 50% of frame, excessive whitespace, off-center composition, product pushed to bottom or top, visible watermarks, shop URLs, overlay text, photo credits.",
