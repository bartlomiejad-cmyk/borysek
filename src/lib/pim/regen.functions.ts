  ...
  .handler(async ({ data, context }) => {
    ...
    const customBlock = (() => {
      ...
      return (
        ...
        parts.join(" | ") +
        " — but the WHITE #FFFFFF background, product colour fidelity, label/logo preservation and 70–80% framing rules ABOVE take absolute priority; if a hint conflicts with them, IGNORE the hint."
      );
    })();
    try {
      shot = await callFal(
